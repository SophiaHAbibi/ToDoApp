...
//Create New Todo Plus Buttons in a row
let mode = "new";
let currentTodo;
...
const renderList = () => {
  ...
  todoStore.forEach((todo, index) => {
    ...
    <tr class="${todo.done ? "completed" : ""}">
      ...
      <td>${todo.subTitle}</td>
      <td>${todo.priority}</td>
      <td>${editBtn(index)} ${dltBtn(index)} ${checkBtn(index, todo.done)}</td>
    </tr>
    `;
...
const validateForm = () => {
  if (!inputTitle.value) return alert("You shouldn't leave Title empty!");
  return true;
};
